...
- **Modernize the ~18 stale-selector e2e specs** — rewrite `e2e/pages/AdminPage.ts` to the real `data-testid`s (unblocks ~32 tests), then `allowance`/`history` `combobox → role="tab"` (+ the single-kid nuance), then harden `ChoresPage`/`RewardsPage`/`HomePage`. ~1 focused day, ~3–6 files, low risk. **[M · high]**
