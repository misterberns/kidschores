    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <TabList label="View mode" className="pb-0">
          <Tab shape="soft" selected={viewMode === 'today'} onClick={() => setViewMode('today')}>
            <Calendar size={16} />
            Today
          </Tab>
          <Tab shape="soft" selected={viewMode === 'all'} onClick={() => setViewMode('all')}>
            <List size={16} />
            All
          </Tab>
        </TabList>
      ...
      {viewMode === 'today' && kids.length > 1 && (
        <TabList label="Select kid">
          {kids.map(kid => (
            <Tab key={kid.id} tone="accent" selected={activeKidId === kid.id} onClick={() => setSelectedKid(kid.id)}>
              {kid.name}
            </Tab>
          ))}
        </TabList>
